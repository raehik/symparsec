...
type Run :: PParser u a -> u -> Symbol -> Either TE.ErrorMessage (a, Symbol)
type Run p user str = RunEnd str (p @@ StateInit user str)

-- | Run a parser on a 'Symbol'. The parser must have empty '()' user state.
type Run' :: PParser () a -> Symbol -> Either TE.ErrorMessage (a, Symbol)
type Run' p str = Run p '() str

type RunEnd :: Symbol -> PReply u a -> Either TE.ErrorMessage (a, Symbol)
type family RunEnd str rep where
    RunEnd str ('Reply (OK  a) ('State  rem _len _idx _user)) =
        ...
        Right '(a, rem)
    RunEnd str ('Reply (Err e) ('State _rem _len  idx _user)) =
...
type RunTest :: PParser u a -> u -> Symbol -> (a, Symbol)
type RunTest p user str = FromRightTypeError (Run p user str)

-- | Run the given parser on the given 'Symbol', emitting a type error on
--   failure. The parser must have empty '()' user state.
...
type StateInit :: u -> Symbol -> PState u
type StateInit user str = 'State str (Symbol.Length str) 0 user
