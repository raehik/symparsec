...
type family LiteralCheckLen lit s n where
    LiteralCheckLen lit ('State rem len idx user) litLen =
        IfNatLte litLen len
            (LiteralStep lit ('State rem len idx user))
            ('Reply (Err (ETooShort lit litLen len)) ('State rem len idx user))
